...
def binSe(kumpulan, target):
    ...
    while low <= high:
        ...
        # Ataukah targetnya di sebelah kanannya?
            ...
            low = mid + 1
    # Jika runtutnya tidak bisa dibelah ;agi, berarti targetnya tidak ada
    return False
...
def cariTerkecil(kumpulan):
    ...
    for i in range(1,n):
        if kumpulan[i].uangSaku < terkecil[0].uangSaku:
            terkecil = [kumpulan[i]]
        elif kumpulan[i].uangSaku == terkecil[0].uangSaku:
            terkecil.append(kumpulan[i])
    return terkecil

# Nomor 4
def UangSakuKurang(kumpulan):
    b = []  
    for i in kumpulan:
